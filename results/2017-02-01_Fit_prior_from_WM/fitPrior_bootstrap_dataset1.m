function fitPrior_bootstrap_dataset1(stimType,outputDir)
...
d2 = load('/data/jet/mattar/mTurk/Scripts/B_colorsAndFaces_101subj.mat');
subjIDs = fieldnames(d2.data);
numSubj2 = length(subjIDs);
for s=1:numSubj2
    thisSubj = subjIDs{s};
    % Block 1
    numTrials = length(d2.data.(thisSubj).(['workingMemory' stimType]).targetVal);
    subjNum = [subjNum; repmat(s+numSubj1,numTrials,1)];
    blockNum = [blockNum; 1*ones(numTrials,1)];
    targetVal = [targetVal; d2.data.(thisSubj).(['workingMemory' stimType]).targetVal];
    responseVal = [responseVal; d2.data.(thisSubj).(['workingMemory' stimType]).responseVal];
    % Block 2
    numTrials = length(d2.data.(thisSubj).(['workingMemory' stimType '2']).targetVal);
    subjNum = [subjNum; repmat(s+numSubj1,numTrials,1)];
    blockNum = [blockNum; 2*ones(numTrials,1)];
    targetVal = [targetVal; d2.data.(thisSubj).(['workingMemory' stimType '2']).targetVal];
    responseVal = [responseVal; d2.data.(thisSubj).(['workingMemory' stimType '2']).responseVal];
end
...
for i=1:length(uniqueTargets)
    ...
    r = wrap(respBoot(targBoot==uniqueTargets(i))*pi/180);
    t = wrap(targBoot(targBoot==uniqueTargets(i))*pi/180);
    e = wrap(r-t);
