(function() {
	...
	var faceArray = loadCSVfile(params.workingMemoryFace.stimulusList);
	// Load the trial sequence from external file
	var temp = loadCSVfile(params.workingMemoryFace.stimulusSeq);
	...
	var temp = loadCSVfile(params.workingMemoryFace.wheelRotation);
	...
	var preExperInstruction = loadTXTfile(params.workingMemoryFace.instructionsPre);
	var breakInstruction = loadTXTfile(params.workingMemoryFace.instructionsBreak);
	var postExperInstruction = loadTXTfile(params.workingMemoryFace.instructionsPost);
	
	var totalTrials = params.workingMemoryFace.totalTrials;
	var trialsPerBlock = params.workingMemoryFace.trialsPerBlock;
	var screenHeight = screen.height;
	var windowHeight = $(window).height();
	var windowWidth = $(window).width();
	var divHeight = $("#content").height();
	var circleSize = divHeight * params.workingMemoryFace.wheelSize;
	var centerSize = circleSize * params.workingMemoryFace.centerSize;
	var calibratorSize = circleSize * params.workingMemoryFace.centerSize;
	var thumbSize = circleSize * params.workingMemoryFace.thumbSize;

	var textColor = params.workingMemoryFace.textColor;
	var instructWidth = params.workingMemoryFace.instructWidth;
	var instructHeight = params.workingMemoryFace.instructHeight;
	var fontSize = params.workingMemoryFace.instructFontsize;
	var buttonWidth = params.workingMemoryFace.instructButtonwidth;
	var buttonHeight = params.workingMemoryFace.instructButtonHeight;
	var buttonFontSize = params.workingMemoryFace.instructButtonfontSize;
	var instructShiftFromTop = params.workingMemoryFace.instructShiftFromTop;

	var minimumClickTime = params.workingMemoryFace.minimumClickTime;
	var maxClickTime = params.workingMemoryFace.maxClickTime;
	...
	var trial = 0;
	var dbTableName = params.workingMemoryFace.dbTableName;
	...
	function initializeExperiment() {
		...
		$("#content").css({"background-color": "rgb" +params.workingMemoryFace.backgroundcolor});
		$("#header").css({"background-color": "rgb" +params.workingMemoryFace.backgroundcolor});
		// Reset header text
		$("#header").html("");
		$("#header").hide();
		...
		//Set mask source
		$(".mask").attr('src',params.workingMemoryFace.maskAddress);
		$(".mask").hide();
		// target
	...
	function setStyle(){
		...
		if($(window).width() != windowWidth || $(window).height() != windowHeight){
			...
			circleSize = divHeight * params.workingMemoryFace.wheelSize;
			centerSize = circleSize * params.workingMemoryFace.centerSize;
			calibratorSize = circleSize * params.workingMemoryFace.centerSize;
			thumbSize = circleSize * params.workingMemoryFace.thumbSize;
		}
		// Define the style of the face circle
		$("#progress-container").css({
		    'width': ''+(windowWidth*params.workingMemoryFace.progressWidth)+'px',
		    'position': 'fixed',
		    'left':''+((windowWidth/2) - (windowWidth*params.workingMemoryFace.progressWidth)/2)+'px',
		    'top':params.workingMemoryFace.progressPercentFromTop +'%',
		    'height': windowHeight*params.workingMemoryFace.progressHeight+'px',
		    'background-color':'rgb'+params.workingMemoryFace.progressBackgroundColor
		});
		$("#progress-bar").css({
		  	'background-color':'rgb'+params.workingMemoryFace.progressForegroundColor,
		  	'width': '0%',
		  	'height': windowHeight*params.workingMemoryFace.progressHeight+'px',
		});
...
$("#directions2").css({
	...
	function executeTrial(){
		...
		time_before_blank1 = $.now();
		$(".blank").show();
		setTimeout(function(){$(".blank").hide();time_after_blank1 = $.now(); time_before_target = $.now();$(".target").show();},params.workingMemoryFace.ISI);
		setTimeout(function(){$(".target").hide(); time_after_target = $.now(); time_before_blank2 = $.now(); $(".blank").show();},params.workingMemoryFace.ISI + params.workingMemoryFace.target_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank2 = $.now(); time_before_mask = $.now();$(".mask").show();},params.workingMemoryFace.ISI + params.workingMemoryFace.target_duration + params.workingMemoryFace.blank_after_target_duration);
		setTimeout(function(){$(".mask").hide(); time_after_mask = $.now(); time_before_blank3 = $.now();$(".blank").show();},params.workingMemoryFace.ISI + params.workingMemoryFace.target_duration + params.workingMemoryFace.blank_after_target_duration + params.workingMemoryFace.mask_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank3 = $.now();$(".face-circle").show();initialTime = $.now();if(maxClickTime != 0){$("#progress-container").show();loadProgressBar('progress-bar', maxClickTime, params.workingMemoryFace.progressInterval);setMaxTimer(trial);}

},params.workingMemoryFace.ISI + params.workingMemoryFace.target_duration + params.workingMemoryFace.blank_after_target_duration + params.workingMemoryFace.mask_duration + params.workingMemoryFace.blank_after_mask);
	}
	function finalizeTrial(responseDegrees) {
		...
		writeTrialData(expData,trial,dbTableName, params.workingMemoryFace.database);
		trial++;
	...
	function takeBreak(){
		$( ".face-circle" ).hide();
		if(trial == 0){
			if(params.workingMemoryFace.displayExperNum == 1){setMessage("This is task " + currentExperNum + "/" + protocolList.length + "\n\n"+ preExperInstruction);}else{setMessage(preExperInstruction);}
	...
	$("#messageButton").click(function() {
		$(".messagedisplay").hide();
		if(trial == 0 && params.workingMemoryFace.instructLoop != undefined){
			instructLoop(params.workingMemoryFace.instructLoop.questions,
			params.workingMemoryFace.instructLoop.choices,
			params.workingMemoryFace.instructLoop.answers,initializeTrial);
			loopstatus = 1;
		}else{
			initializeTrial();
		}
