(function() {
	...
	var RGBarray = loadCSVfile(params.primeColorShort.stimulusList);
	
	// Load the trial sequence from external file
	var temp = loadCSVfile(params.primeColorShort.stimulusSeq);
	...
	var temp = loadCSVfile(params.primeColorShort.primeSeq);
	var seqLen = temp.length;
	var primeSeq = [];
	...
	var temp = loadCSVfile(params.primeColorShort.wheelRotation);
	...
	var preExperInstruction = loadTXTfile(params.primeColorShort.instructionsPre);
	var breakInstruction = loadTXTfile(params.primeColorShort.instructionsBreak);
	var postExperInstruction = loadTXTfile(params.primeColorShort.instructionsPost);

	var trialsPerBlock = params.primeColorShort.trialsPerBlock;
	var totalTrials = params.primeColorShort.totalTrials;
	var screenHeight = screen.height;
	var windowHeight = $(window).height();
	var windowWidth = $(window).width();
	var divHeight = $("#content").height();
	var circleSize = divHeight * params.primeColorShort.wheelSize;
	var centerSize = circleSize * params.primeColorShort.centerSize;
	var calibratorSize = circleSize * params.primeColorShort.centerSize;
	var thumbSize = circleSize * params.primeColorShort.thumbSize;

	var textColor = params.primeColorShort.textColor;
	var instructWidth = params.primeColorShort.instructWidth;
	var instructHeight = params.primeColorShort.instructHeight;
	var fontSize = params.primeColorShort.instructFontsize;
	var buttonWidth = params.primeColorShort.instructButtonwidth;
	var buttonHeight = params.primeColorShort.instructButtonHeight;
	var buttonFontSize = params.primeColorShort.instructButtonfontSize;
	var instructShiftFromTop = params.primeColorShort.instructShiftFromTop;

	var minimumClickTime = params.primeColorShort.minimumClickTime;
	var maxClickTime = params.primeColorShort.maxClickTime;
	var primeDetect = params.primeColorShort.primeDetect;
	...
	var trial = 0;
	var dbTableName = params.primeColorShort.dbTableName;
	...
	function initializeExperiment() {
		...
		$("#content").css({"background-color": "rgb" +params.primeColorShort.backgroundcolor});
		$("#header").css({"background-color": "rgb" +params.primeColorShort.backgroundcolor});
		// Reset header text
		$("#header").html("");
		$("#header").hide();
		...
		//Set mask source
		$(".mask").attr('src',params.primeColorShort.maskAddress);
		$(".mask").hide();
		// target
	...
	function setStyle(){
		...
		if($(window).width() != windowWidth || $(window).height() != windowHeight){
			...
			circleSize = divHeight * params.primeColorShort.wheelSize;
			centerSize = circleSize * params.primeColorShort.centerSize;
			calibratorSize = circleSize * params.primeColorShort.centerSize;
			thumbSize = circleSize * params.primeColorShort.thumbSize;
		}
		// Define the style of the color circle
		$("#progress-container").css({
		    'width': ''+(windowWidth*params.primeColorShort.progressWidth)+'px',
		    'position': 'fixed',
		    'left':''+((windowWidth/2) - (windowWidth*params.primeColorShort.progressWidth)/2)+'px',
		    'top':params.primeColorShort.progressPercentFromTop +'%',
		    'height': windowHeight*params.primeColorShort.progressHeight+'px',
		    'background-color':'rgb'+params.primeColorShort.progressBackgroundColor
		});
		$("#progress-bar").css({
		  	'background-color':'rgb'+params.primeColorShort.progressForegroundColor,
		  	'width': '0%',
		  	'height': windowHeight*params.primeColorShort.progressHeight+'px',
		});
...
$("#directions2").css({
	...
	function executeTrial(){
		...
		$(".blank").show();
		setTimeout(function(){$(".blank").hide();time_after_blank0 = $.now(); time_before_prime = $.now();$(".prime").show();}, params.primeColorShort.blank_before_prime_duration);
		setTimeout(function(){$(".prime").hide();time_after_prime = $.now(); time_before_blank1 = $.now();$(".blank").show();}, params.primeColorShort.blank_before_prime_duration + params.primeColorShort.prime_duration);
		setTimeout(function(){$(".blank").hide();time_after_blank1 = $.now(); time_before_target = $.now();$(".target").show();}, params.primeColorShort.blank_before_prime_duration + params.primeColorShort.prime_duration + params.primeColorShort.ISI);
		setTimeout(function(){$(".target").hide(); time_after_target = $.now(); time_before_blank2 = $.now(); $(".blank").show();}, params.primeColorShort.blank_before_prime_duration + params.primeColorShort.prime_duration + params.primeColorShort.ISI + params.primeColorShort.target_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank2 = $.now(); time_before_mask = $.now();$(".mask").show();}, params.primeColorShort.blank_before_prime_duration + params.primeColorShort.prime_duration+ params.primeColorShort.ISI + params.primeColorShort.target_duration + params.primeColorShort.blank_after_target_duration);
		setTimeout(function(){$(".mask").hide(); time_after_mask = $.now(); time_before_blank3 = $.now();$(".blank").show();}, params.primeColorShort.blank_before_prime_duration + params.primeColorShort.prime_duration+ params.primeColorShort.ISI + params.primeColorShort.target_duration + params.primeColorShort.blank_after_target_duration + params.primeColorShort.mask_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank3 = $.now();$(".color-circle").show();initialTime = $.now();if(maxClickTime != 0){$("#progress-container").show();loadProgressBar('progress-bar', maxClickTime, params.primeColorShort.progressInterval);setMaxTimer(trial);}

},params.primeColorShort.blank_before_prime_duration + params.primeColorShort.prime_duration + params.primeColorShort.ISI + params.primeColorShort.target_duration + params.primeColorShort.blank_after_target_duration + params.primeColorShort.mask_duration + params.primeColorShort.blank_after_mask);
	}
	function finalizeTrial(responseDegrees) {
		...
		writeTrialData(expData,trial,dbTableName, params.primeColorShort.database);
		trial++;
	...
	function takeBreak(){
		$( ".color-circle" ).hide();
		if(trial == 0){
			if(params.primeColorShort.displayExperNum == 1){setMessage("This is task " + currentExperNum + "/" + protocolList.length + "\n\n"+ preExperInstruction);}else{setMessage(preExperInstruction);}
	...
	$("#messageButton").click(function() {
		$(".messagedisplay").hide();
		if(trial == 0 && params.primeColorShort.instructLoop != undefined){
			instructLoop(params.primeColorShort.instructLoop.questions,
			params.primeColorShort.instructLoop.choices,
			params.primeColorShort.instructLoop.answers,initializeTrial);
			loopstatus = 1;
		}else{
			initializeTrial();
		}
