(function() {
	...
	$("#header").hide();
	var backgroundcolor = params.screenCalibration.backgroundcolor;
	var textcolor = params.screenCalibration.textColor;
	var promptWidth = params.screenCalibration.promptWidth;
	var promptHeight = params.screenCalibration.promptHeight;
	var fontSize = params.screenCalibration.fontsize;
	var buttonWidth = params.screenCalibration.buttonwidth;
	var buttonHeight = params.screenCalibration.buttonHeight;
	var buttonFontSize = params.screenCalibration.buttonfontSize;
	var textBoxHeight = params.screenCalibration.textBoxHeight;
	var textBoxWidth = params.screenCalibration.textBoxWidth;
	var textBoxbgColor = params.screenCalibration.textBoxbgColor;
	var imageWidth = params.screenCalibration.imageWidth;
	var imageHeight = params.screenCalibration.imageHeight;
	var tableName = params.screenCalibration.tableName;
	var fieldName = params.screenCalibration.fieldName;

	var ishSeq = params.screenCalibration.ishSeq;
	var ishAnswers = params.screenCalibration.ishAnswers;
	...
	imageWidth = imageWidth*windowWidth;
	...
	function initializeExperiment() {
		...
		// Append screenCalibration task 
		$("#content").append("<div class = 'screenCalibration'>\
		<img id = 'screenCalibrationImg' draggable = 'false' src = "+params.screenCalibration.calibrationImgSrc+"></img>'\
		<br><br>\
	    <label id='screenCalibrationPrompt'>The following image shows scales of linearly increasing RGB values. You should use these images to adjust your monitor settings. \nOn a good monitor, you should see roughly equal steps in brightness over the full range from 1 to 32, and in all colors. Bar 1 should be visible. A bad monitor will not show the leftmost few rectangles, will not show a difference between the rightmost few, or shows a relatively large jump between numbers 31 and 32.\nPlease, adjust your screen settings (e.g. brightness, screen tilt, etc) until you can see all bars and click the button below when you are done.</label>\
	    <br>\
	    <button id='screenCalibrationButton'>Done Calibrating!</button>\
		</div>");
	...
	$("#screenCalibrationButton").click(function() {
		finalizeExperiment();
	});
