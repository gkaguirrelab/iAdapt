(function() {
	...
	var faceArray = loadCSVfile(params.primeFaceShort.stimulusList);
	// Load the trial sequence from external file
	var temp = loadCSVfile(params.primeFaceShort.stimulusSeq);
	...
	var temp = loadCSVfile(params.primeFaceShort.primeSeq);
	var seqLen = temp.length;
	var primeSeq = [];
	...
	var temp = loadCSVfile(params.primeFaceShort.wheelRotation);
	...
	var preExperInstruction = loadTXTfile(params.primeFaceShort.instructionsPre);
	var breakInstruction = loadTXTfile(params.primeFaceShort.instructionsBreak);
	var postExperInstruction = loadTXTfile(params.primeFaceShort.instructionsPost);

	var totalTrials = params.primeFaceShort.totalTrials;
	var trialsPerBlock = params.primeFaceShort.trialsPerBlock;
	var screenHeight = screen.height;
	var windowHeight = $(window).height();
	var windowWidth = $(window).width();
	var divHeight = $("#content").height();
	var circleSize = divHeight * params.primeFaceShort.wheelSize;
	var centerSize = circleSize * params.primeFaceShort.centerSize;
	var calibratorSize = circleSize * params.primeFaceShort.centerSize;
	var thumbSize = circleSize * params.primeFaceShort.thumbSize;

	var textColor = params.primeFaceShort.textColor;
	var instructWidth = params.primeFaceShort.instructWidth;
	var instructHeight = params.primeFaceShort.instructHeight;
	var fontSize = params.primeFaceShort.instructFontsize;
	var buttonWidth = params.primeFaceShort.instructButtonwidth;
	var buttonHeight = params.primeFaceShort.instructButtonHeight;
	var buttonFontSize = params.primeFaceShort.instructButtonfontSize;
	var instructShiftFromTop = params.primeFaceShort.instructShiftFromTop;

	var minimumClickTime = params.primeFaceShort.minimumClickTime;
	var maxClickTime = params.primeFaceShort.maxClickTime;
	var primeDetect = params.primeFaceShort.primeDetect;
	...
	var trial = 0;
	var dbTableName = params.primeFaceShort.dbTableName;
	...
	function initializeExperiment() {
		...
		$("#content").css({"background-color": "rgb" +params.primeFaceShort.backgroundcolor});
		$("#header").css({"background-color": "rgb" +params.primeFaceShort.backgroundcolor});
		// Reset header text
		$("#header").html("");
		$("#header").hide();
		...
		//Set mask source
		$(".mask").attr('src',params.primeFaceShort.maskAddress);
		$(".mask").hide();
		// target
	...
	function setStyle(){
		...
		if($(window).width() != windowWidth || $(window).height() != windowHeight){
			...
			circleSize = divHeight * params.primeFaceShort.wheelSize;
			centerSize = circleSize * params.primeFaceShort.centerSize;
			calibratorSize = circleSize * params.primeFaceShort.centerSize;
			thumbSize = circleSize * params.primeFaceShort.thumbSize;
		}
		// Define the style of the face circle
		$("#progress-container").css({
		    'width': ''+(windowWidth*params.primeFaceShort.progressWidth)+'px',
		    'position': 'fixed',
		    'left':''+((windowWidth/2) - (windowWidth*params.primeFaceShort.progressWidth)/2)+'px',
		    'top':params.primeFaceShort.progressPercentFromTop +'%',
		    'height': windowHeight*params.primeFaceShort.progressHeight+'px',
		    'background-color':'rgb'+params.primeFaceShort.progressBackgroundColor
		});
		$("#progress-bar").css({
		  	'background-color':'rgb'+params.primeFaceShort.progressForegroundColor,
		  	'width': '0%',
		  	'height': windowHeight*params.primeFaceShort.progressHeight+'px',
		});
...
$("#directions2").css({
	...
	function executeTrial(){
		...
		$(".blank").show();
		setTimeout(function(){$(".blank").hide();time_after_blank0 = $.now(); time_before_prime = $.now();$(".prime").show();}, params.primeFaceShort.blank_before_prime_duration);
		setTimeout(function(){$(".prime").hide();time_after_prime = $.now(); time_before_blank1 = $.now();$(".blank").show();}, params.primeFaceShort.blank_before_prime_duration + params.primeFaceShort.prime_duration);
		setTimeout(function(){$(".blank").hide();time_after_blank1 = $.now(); time_before_target = $.now();$(".target").show();}, params.primeFaceShort.blank_before_prime_duration + params.primeFaceShort.prime_duration + params.primeFaceShort.ISI);
		setTimeout(function(){$(".target").hide(); time_after_target = $.now(); time_before_blank2 = $.now(); $(".blank").show();}, params.primeFaceShort.blank_before_prime_duration + params.primeFaceShort.prime_duration + params.primeFaceShort.ISI + params.primeFaceShort.target_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank2 = $.now(); time_before_mask = $.now();$(".mask").show();}, params.primeFaceShort.blank_before_prime_duration + params.primeFaceShort.prime_duration+ params.primeFaceShort.ISI + params.primeFaceShort.target_duration + params.primeFaceShort.blank_after_target_duration);
		setTimeout(function(){$(".mask").hide(); time_after_mask = $.now(); time_before_blank3 = $.now();$(".blank").show();}, params.primeFaceShort.blank_before_prime_duration + params.primeFaceShort.prime_duration+ params.primeFaceShort.ISI + params.primeFaceShort.target_duration + params.primeFaceShort.blank_after_target_duration + params.primeFaceShort.mask_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank3 = $.now();$(".face-circle").show();initialTime = $.now();if(maxClickTime != 0){$("#progress-container").show();loadProgressBar('progress-bar', maxClickTime, params.primeFaceShort.progressInterval);setMaxTimer(trial);}

},params.primeFaceShort.blank_before_prime_duration + params.primeFaceShort.prime_duration + params.primeFaceShort.ISI + params.primeFaceShort.target_duration + params.primeFaceShort.blank_after_target_duration + params.primeFaceShort.mask_duration + params.primeFaceShort.blank_after_mask);
	}
	function finalizeTrial(responseDegrees) {
		...
		writeTrialData(expData,trial,dbTableName, params.primeFaceShort.database);
		trial++;
	...
	function takeBreak(){
		$( ".face-circle" ).hide();
		if(trial == 0){
			if(params.primeFaceShort.displayExperNum == 1){setMessage("This is task " + currentExperNum + "/" + protocolList.length + "\n\n"+ preExperInstruction);}else{setMessage(preExperInstruction);}
	...
	$("#messageButton").click(function() {
		$(".messagedisplay").hide();
		if(trial == 0 && params.primeFaceShort.instructLoop != undefined){
			instructLoop(params.primeFaceShort.instructLoop.questions,
			params.primeFaceShort.instructLoop.choices,
			params.primeFaceShort.instructLoop.answers,initializeTrial);
			loopstatus = 1;
		}else{
			initializeTrial();
		}
	...
	$("html").click(function(){
		if($(".color-circle").is(":visible")){
			...
			/*
				...
				if(task == "primeColorLong" || task == "primeFaceShort"){
					...
					if(task == "primeColorLong"){
						...
						var target = params["primeprimeFaceShort"][taskTrialNum - 2] + rotation;
						if (target > 360) target -= 360;
						if (target < 0) target += 360;
						currPrime = calcPrimeOffset(target, params["primeprimeFaceShort"][taskTrialNum - 2]);
					}
