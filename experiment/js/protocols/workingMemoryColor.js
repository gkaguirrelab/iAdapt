(function() {
	...
	var RGBarray = loadCSVfile(params.workingMemoryColor.stimulusList);
	

	// Load the trial sequence from external file
	var temp = loadCSVfile(params.workingMemoryColor.stimulusSeq);
	...
	var temp = loadCSVfile(params.workingMemoryColor.wheelRotation);
	...
	var preExperInstruction = loadTXTfile(params.workingMemoryColor.instructionsPre);
	var breakInstruction = loadTXTfile(params.workingMemoryColor.instructionsBreak);
	var postExperInstruction = loadTXTfile(params.workingMemoryColor.instructionsPost);
	
	var trialsPerBlock = params.workingMemoryColor.trialsPerBlock;
	var totalTrials = params.workingMemoryColor.totalTrials;
	var screenHeight = screen.height;
	var windowHeight = $(window).height();
	var windowWidth = $(window).width();
	var divHeight = $("#content").height();
	var circleSize = divHeight * params.workingMemoryColor.wheelSize;
	var centerSize = circleSize * params.workingMemoryColor.centerSize;
	var calibratorSize = circleSize * params.workingMemoryColor.centerSize;
	var thumbSize = circleSize * params.workingMemoryColor.thumbSize;

	var textColor = params.workingMemoryColor.textColor;
	var instructWidth = params.workingMemoryColor.instructWidth;
	var instructHeight = params.workingMemoryColor.instructHeight;
	var fontSize = params.workingMemoryColor.instructFontsize;
	var buttonWidth = params.workingMemoryColor.instructButtonwidth;
	var buttonHeight = params.workingMemoryColor.instructButtonHeight;
	var buttonFontSize = params.workingMemoryColor.instructButtonfontSize;
	var instructShiftFromTop = params.workingMemoryColor.instructShiftFromTop;

	var minimumClickTime = params.workingMemoryColor.minimumClickTime;
	var maxClickTime = params.workingMemoryColor.maxClickTime;
	...
	var trial = 0;
	var dbTableName = params.workingMemoryColor.dbTableName;
	...
	function initializeExperiment() {
		...
		$("#content").css({"background-color": "rgb" +params.workingMemoryColor.backgroundcolor});
		$("#header").css({"background-color": "rgb" +params.workingMemoryColor.backgroundcolor});
		// Reset header text
		$("#header").html("");
		$("#header").hide();
		...
		//Set mask source
		$(".mask").attr('src',params.workingMemoryColor.maskAddress);
		$(".mask").hide();
		// target
	...
	function setStyle(){
		...
		if($(window).width() != windowWidth || $(window).height() != windowHeight){
			...
			circleSize = divHeight * params.workingMemoryColor.wheelSize;
			centerSize = circleSize * params.workingMemoryColor.centerSize;
			calibratorSize = circleSize * params.workingMemoryColor.centerSize;
			thumbSize = circleSize * params.workingMemoryColor.thumbSize;
		}
		// Define the style of the color circle
		$("#progress-container").css({
		    'width': ''+(windowWidth*params.workingMemoryColor.progressWidth)+'px',
		    'position': 'fixed',
		    'left':''+((windowWidth/2) - (windowWidth*params.workingMemoryColor.progressWidth)/2)+'px',
		    'top':params.workingMemoryColor.progressPercentFromTop +'%',
		    'height': windowHeight*params.workingMemoryColor.progressHeight+'px',
		    'background-color':'rgb'+params.workingMemoryColor.progressBackgroundColor
		});
		$("#progress-bar").css({
		  	'background-color':'rgb'+params.workingMemoryColor.progressForegroundColor,
		  	'width': '0%',
		  	'height': windowHeight*params.workingMemoryColor.progressHeight+'px',
		});
...
$("#directions2").css({
	...
	function executeTrial(){
		...
		$(".blank").show();
		setTimeout(function(){$(".blank").hide();time_after_blank1 = $.now(); time_before_target = $.now();$(".target").show();},params.workingMemoryColor.ISI);
		setTimeout(function(){$(".target").hide(); time_after_target = $.now(); time_before_blank2 = $.now(); $(".blank").show();}, params.workingMemoryColor.ISI + params.workingMemoryColor.target_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank2 = $.now(); time_before_mask = $.now();$(".mask").show();},params.workingMemoryColor.ISI + params.workingMemoryColor.target_duration + params.workingMemoryColor.blank_after_target_duration);
		setTimeout(function(){$(".mask").hide(); time_after_mask = $.now(); time_before_blank3 = $.now();$(".blank").show();},params.workingMemoryColor.ISI + params.workingMemoryColor.target_duration + params.workingMemoryColor.blank_after_target_duration + params.workingMemoryColor.mask_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank3 = $.now();$(".color-circle").show();initialTime = $.now();if(maxClickTime != 0){$("#progress-container").show();loadProgressBar('progress-bar', maxClickTime, params.workingMemoryColor.progressInterval);setMaxTimer(trial);}

},params.workingMemoryColor.ISI + params.workingMemoryColor.target_duration + params.workingMemoryColor.blank_after_target_duration + params.workingMemoryColor.mask_duration + params.workingMemoryColor.blank_after_mask);
	}
	function finalizeTrial(responseDegrees) {
		...
		writeTrialData(expData,trial,dbTableName, params.workingMemoryColor.database);
		trial++;
	...
	function takeBreak(){
		$( ".color-circle" ).hide();
		if(trial == 0){
			if(params.workingMemoryColor.displayExperNum == 1){setMessage("This is task " + currentExperNum + "/" + protocolList.length + "\n\n"+ preExperInstruction);}else{setMessage(preExperInstruction);}
	...
	$("#messageButton").click(function() {
		$(".messagedisplay").hide();
		if(trial == 0 && params.workingMemoryColor.instructLoop != undefined){
			instructLoop(params.workingMemoryColor.instructLoop.questions,
			params.workingMemoryColor.instructLoop.choices,
			params.workingMemoryColor.instructLoop.answers,initializeTrial);
			loopstatus = 1;
		}else{
			initializeTrial();
		}
