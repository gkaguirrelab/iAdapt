(function() {
	...
	var RGBarray = loadCSVfile(params.calibrationColor.stimulusList);
	

	// Load the trial sequence from external file
	var temp = loadCSVfile(params.calibrationColor.stimulusSeq);
	...
	var temp = loadCSVfile(params.calibrationColor.wheelRotation);
	...
	var preExperInstruction = loadTXTfile(params.calibrationColor.instructionsPre);
	var breakInstruction = loadTXTfile(params.calibrationColor.instructionsBreak);
	var postExperInstruction = loadTXTfile(params.calibrationColor.instructionsPost);

	var trialsPerBlock = params.calibrationColor.trialsPerBlock;
	var totalTrials = params.calibrationColor.totalTrials;
	var screenHeight = screen.height;
	var windowHeight = $(window).height();
	var windowWidth = $(window).width();
	var divHeight = $("#content").height();
	var circleSize = divHeight * params.calibrationColor.wheelSize;
	var centerSize = circleSize * params.calibrationColor.centerSize;
	var calibratorSize = circleSize * params.calibrationColor.centerSize;
	var thumbSize = circleSize * params.calibrationColor.thumbSize;
	
	var textColor = params.calibrationColor.textColor;
	var instructWidth = params.calibrationColor.instructWidth;
	var instructHeight = params.calibrationColor.instructHeight;
	var fontSize = params.calibrationColor.instructFontsize;
	var buttonWidth = params.calibrationColor.instructButtonwidth;
	var buttonHeight = params.calibrationColor.instructButtonHeight;
	var buttonFontSize = params.calibrationColor.instructButtonfontSize;
	var instructShiftFromTop = params.calibrationColor.instructShiftFromTop;

	var minimumClickTime = params.calibrationColor.minimumClickTime;
	var maxClickTime = params.calibrationColor.maxClickTime;

	// Accumulates the subject's accuracy score, to be used 
	// in order to calculate average accuracy score
	var accuracySum = 0;

	// status variables are used to manage event functions
	var breakstatus = 0;
	var clickstatus = 0;
	var loopstatus = 0;

	var trial = 0;
	var dbTableName = params.calibrationColor.dbTableName;
	...
	function initializeExperiment() {
		...
		$("#content").css({"background-color": "rgb" +params.calibrationColor.backgroundcolor});
		$("#header").css({"background-color": "rgb" +params.calibrationColor.backgroundcolor});
		// Reset header text
		$("#header").html("");
		$("#header").hide();
		...
		$("#content").append("<div class = 'color-circle'>\
		    ...
		    <div class = 'thumbColors' id = 'colorCalibratorLeft'></div>\
		    <div class = 'thumbColors' id = 'colorCalibratorRight'></div>\
	...
	function setStyle(){
		...
		if($(window).width() != windowWidth || $(window).height() != windowHeight){
			...
			circleSize = divHeight * params.calibrationColor.wheelSize;
			centerSize = circleSize * params.calibrationColor.centerSize;
			calibratorSize = circleSize * params.calibrationColor.centerSize;
			thumbSize = circleSize * params.calibrationColor.thumbSize;
		}
		// Define the style of the color circle
		$("#progress-container").css({
		    'width': ''+(windowWidth*params.calibrationColor.progressWidth)+'px',
		    'position': 'fixed',
		    'left':''+((windowWidth/2) - (windowWidth*params.calibrationColor.progressWidth)/2)+'px',
		    'top':params.calibrationColor.progressPercentFromTop +'%',
		    'height': windowHeight*params.calibrationColor.progressHeight+'px',
		    'background-color':'rgb'+params.calibrationColor.progressBackgroundColor
		});
		$("#progress-bar").css({
		  	'background-color':'rgb'+params.calibrationColor.progressForegroundColor,
		  	'width': '0%',
		  	'height': windowHeight*params.calibrationColor.progressHeight+'px',
		});
	...
	function executeTrial(){
		...
		if(maxClickTime != 0){
			...
			loadProgressBar('progress-bar', maxClickTime, params.calibrationColor.progressInterval);	
			setMaxTimer(trial);
		}
	...
	function finalizeTrial(responseDegrees) {
		...
		writeTrialData(expData,trial,dbTableName, params.calibrationColor.database);
		trial++;
	...
	function takeBreak(){
		$( ".color-circle" ).hide();
		if(trial == 0){
			if(params.calibrationColor.displayExperNum == 1){setMessage("This is task " + currentExperNum + "/" + protocolList.length + "\n\n"+ preExperInstruction);}else{setMessage(preExperInstruction);}
	...
	$("#messageButton").click(function() {
		$(".messagedisplay").hide();
		if(trial == 0 && params.calibrationColor.instructLoop != undefined){
			instructLoop(params.calibrationColor.instructLoop.questions,
			params.calibrationColor.instructLoop.choices,
			params.calibrationColor.instructLoop.answers,initializeTrial);
			loopstatus = 1;
		}else{
			initializeTrial();
		}
...
//console.log("calibrationColor.js successfully executed");
