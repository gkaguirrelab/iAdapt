(function() {
	...
	//Load instructions
	var instructions = loadTXTfile(instructs.completionInstructions.instructions);
	var screenHeight = screen.height;
	var windowHeight = $(window).height();
	var windowWidth = $(window).width();
	var divHeight = $("#content").height();

	var backgroundcolor = instructs.completionInstructions.backgroundcolor;
	var textcolor = instructs.completionInstructions.textColor;
	var instructWidth = instructs.completionInstructions.width;
	var instructHeight = instructs.completionInstructions.height;
	var fontSize = instructs.completionInstructions.fontsize;
	...
	// Displays a message to the user as well as a completion code
	function codeMessage(code){
		$("#messagedisplay").text("" + instructions + code); 
		$(".messagedisplay").show();
	}
	...
	function getCompletionCode () {
		$.ajax({
			type: "POST",
			url: "database/completionCode.php",
			data: {'userID': userID , 'DB': instructs.completionInstructions.database},
...
function calibrationColor(){
			...
			/*
				...
				if(task == "primeColorLong" || task == "primeColorShort"){
					...
					if(task == "primeColorLong"){
						var target = instructs["primeTargetColorLong"][taskTrialNum - 2] + rotation;
						if (target > 360) target -= 360;
						if (target < 0) target += 360;
						currPrime = calcPrimeOffset(target, instructs["primePrimeColorLong"][taskTrialNum - 2]);
					}
					else{
						var target = instructs["primePrimeColorShort"][taskTrialNum - 2] + rotation;
						if (target > 360) target -= 360;
						if (target < 0) target += 360;
						currPrime = calcPrimeOffset(target, instructs["primePrimeColorShort"][taskTrialNum - 2]);
					}
				...
				rotateWheel((rotation * -1),"color",instructs["initialize"][0]);
				trial();
			}
			*/
		}
	...
	if(taskTrialNum <= thisSeq.length){
		if(blockNum == -1){
			...
			setDirections(instructs.calibrationColor.instructionsPre);
			$(".directions").show();
			blockNum ++;
		...
		}else if(taskTrialNum <= instructs.calibrationColor.trialsPerBlock * blockNum){
			...
			setTimeout(function(){rotateWheel(wheelRotation[taskTrialNum-1][1],"color",instructs.calibrationColor.wheelSize);},0);
			...
			setTimeout(function(){$(".colorCenter").hide(); $(".faceCenter").hide();}, 0);
			setTimeout(function(){setDirections(instructs.calibrationColor.instructionsBreak);}, 0);
		...
		setTimeout(function(){setDirections(instructs.calibrationColor.instructionsPost);}, 0);
		setTimeout(function(){$(".directions").show();}, 0);
		blockNum = -1;
		taskTrialNum = 1;
		//Next task
...
setThumbNailSize(instructs["initialize"][1]); // In setGetters.js
// Sets height and width both to a percentage of the screen height
setCircleSize(instructs["initialize"][0]); // In setGetters.js
// Sets height and width both to a percentage of the screen height
setCenterImageSize(instructs["initialize"][2]); // In setGetters.js
*/
// Append HTML element
