(function() {
	...
	var RGBarray = loadCSVfile(params.primeColorLong.stimulusList);
	
	// Load the trial sequence from external file
	var temp = loadCSVfile(params.primeColorLong.stimulusSeq);
	...
	var temp = loadCSVfile(params.primeColorLong.primeSeq);
	var seqLen = temp.length;
	var primeSeq = [];
	...
	var temp = loadCSVfile(params.primeColorLong.wheelRotation);
	...
	var preExperInstruction = loadTXTfile(params.primeColorLong.instructionsPre);
	var breakInstruction = loadTXTfile(params.primeColorLong.instructionsBreak);
	var postExperInstruction = loadTXTfile(params.primeColorLong.instructionsPost);

	var trialsPerBlock = params.primeColorLong.trialsPerBlock;
	var totalTrials = params.primeColorLong.totalTrials;
	var screenHeight = screen.height;
	var windowHeight = $(window).height();
	var windowWidth = $(window).width();
	var divHeight = $("#content").height();
	var circleSize = divHeight * params.primeColorLong.wheelSize;
	var centerSize = circleSize * params.primeColorLong.centerSize;
	var calibratorSize = circleSize * params.primeColorLong.centerSize;
	var thumbSize = circleSize * params.primeColorLong.thumbSize;

	var textColor = params.primeColorLong.textColor;
	var instructWidth = params.primeColorLong.instructWidth;
	var instructHeight = params.primeColorLong.instructHeight;
	var fontSize = params.primeColorLong.instructFontsize;
	var buttonWidth = params.primeColorLong.instructButtonwidth;
	var buttonHeight = params.primeColorLong.instructButtonHeight;
	var buttonFontSize = params.primeColorLong.instructButtonfontSize;
	var instructShiftFromTop = params.primeColorLong.instructShiftFromTop;

	var minimumClickTime = params.primeColorLong.minimumClickTime;
	var maxClickTime = params.primeColorLong.maxClickTime;
	var primeDetect = params.primeColorLong.primeDetect;
	...
	var trial = 0;
	var dbTableName = params.primeColorLong.dbTableName;
	...
	function initializeExperiment() {
		...
		$("#content").css({"background-color": "rgb" +params.primeColorLong.backgroundcolor});
		$("#header").css({"background-color": "rgb" +params.primeColorLong.backgroundcolor});
		// Reset header text
		$("#header").html("");
		$("#header").hide();
		...
		//Set mask source
		$(".mask").attr('src',params.primeColorLong.maskAddress);
		$(".mask").hide();
		// target
	...
	function setStyle(){
		...
		if($(window).width() != windowWidth || $(window).height() != windowHeight){
			...
			circleSize = divHeight * params.primeColorLong.wheelSize;
			centerSize = circleSize * params.primeColorLong.centerSize;
			calibratorSize = circleSize * params.primeColorLong.centerSize;
			thumbSize = circleSize * params.primeColorLong.thumbSize;
		}
		// Define the style of the color circle
		$("#progress-container").css({
		    'width': ''+(windowWidth*params.primeColorLong.progressWidth)+'px',
		    'position': 'fixed',
		    'left':''+((windowWidth/2) - (windowWidth*params.primeColorLong.progressWidth)/2)+'px',
		    'top':params.primeColorLong.progressPercentFromTop +'%',
		    'height': windowHeight*params.primeColorLong.progressHeight+'px',
		    'background-color':'rgb'+params.primeColorLong.progressBackgroundColor
		});
		$("#progress-bar").css({
		  	'background-color':'rgb'+params.primeColorLong.progressForegroundColor,
		  	'width': '0%',
		  	'height': windowHeight*params.primeColorLong.progressHeight+'px',
		});
...
$("#directions2").css({
	...
	function executeTrial(){
		...
		$(".blank").show();
		setTimeout(function(){$(".blank").hide();time_after_blank0 = $.now(); time_before_prime = $.now();$(".prime").show();}, params.primeColorLong.blank_before_prime_duration);
		setTimeout(function(){$(".prime").hide();time_after_prime = $.now(); time_before_blank1 = $.now();$(".blank").show();}, params.primeColorLong.blank_before_prime_duration + params.primeColorLong.prime_duration);
		setTimeout(function(){$(".blank").hide();time_after_blank1 = $.now(); time_before_mask = $.now();$(".mask").show();}, params.primeColorLong.blank_before_prime_duration + params.primeColorLong.prime_duration + params.primeColorLong.blank_before_mask);
		setTimeout(function(){$(".mask").hide(); time_after_mask  = $.now(); time_before_blank2 = $.now();$(".blank").show();}, params.primeColorLong.blank_before_prime_duration + params.primeColorLong.prime_duration + params.primeColorLong.blank_before_mask + params.primeColorLong.mask_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank2  = $.now(); time_before_target  = $.now();$(".target").show();}, params.primeColorLong.blank_before_prime_duration + params.primeColorLong.prime_duration+ params.primeColorLong.blank_before_mask + params.primeColorLong.mask_duration + params.primeColorLong.ISI);
		setTimeout(function(){$(".target").hide(); time_after_target = $.now(); time_before_blank3 = $.now(); $(".blank").show();}, params.primeColorLong.blank_before_prime_duration + params.primeColorLong.prime_duration+ params.primeColorLong.blank_before_mask + params.primeColorLong.mask_duration + params.primeColorLong.ISI + params.primeColorLong.target_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank3 = $.now();$(".color-circle").show();initialTime = $.now();if(maxClickTime != 0){$("#progress-container").show();loadProgressBar('progress-bar', maxClickTime, params.primeColorLong.progressInterval);setMaxTimer(trial);}

},params.primeColorLong.blank_before_prime_duration + params.primeColorLong.prime_duration + params.primeColorLong.blank_before_mask + params.primeColorLong.mask_duration + params.primeColorLong.ISI + params.primeColorLong.target_duration + params.primeColorLong.blank_after_target_duration);
	}	
	function finalizeTrial(responseDegrees) {
		...
		writeTrialData(expData,trial,dbTableName, params.primeColorLong.database);
		trial++;
	...
	function takeBreak(){
		...
		if(trial == 0){
			var moneyMadeSoFar = "";
			if(params.primeColorLong.payByPerformance == 1){
				moneyMadeSoFar = "You have made " +totalEarned/100+"$ so far!\n"
			}
			if(params.primeColorLong.displayExperNum == 1){setMessage(moneyMadeSoFar + "This is task " + currentExperNum + "/" + protocolList.length + "\n\n"+ preExperInstruction);}else{setMessage(preExperInstruction);}
			...
			var moneyMadeUpdate = "";
			if(params.primeColorLong.payByPerformance == 1){
				totalEarned += Math.round(parseFloat(Math.max(totalScore, 0) *params.primeColorLong.paymentFull));
				moneyMadeUpdate = "\nYou earned " +Math.round(parseFloat(Math.max(totalScore, 0) *params.primeColorLong.paymentFull))+" cents for this task.";
				if(totalScore < params.primeColorLong.minPerformance){
					poorAccuracy = 1;
				}
			}
	...
	$("#messageButton").click(function() {
		$(".messagedisplay").hide();
		if(trial == 0 && params.primeColorLong.instructLoop != undefined){
			instructLoop(params.primeColorLong.instructLoop.questions,
			params.primeColorLong.instructLoop.choices,
			params.primeColorLong.instructLoop.answers,initializeTrial);
			loopstatus = 1;
		}else{
			initializeTrial();
		}
...
function primeColorLong(){
	...
	$("html").click(function(){
		if($(".color-circle").is(":visible")){
			...
			/*
				...
				if(task == "primeColorLong" || task == "primeColorLong"){
					...
					if(task == "primeColorLong"){
						...
						var target = params["primeprimeColorLong"][taskTrialNum - 2] + rotation;
						if (target > 360) target -= 360;
						if (target < 0) target += 360;
						currPrime = calcPrimeOffset(target, params["primeprimeColorLong"][taskTrialNum - 2]);
					}
	...
	if(taskTrialNum <= thisSeq.length){
		if(blockNum == -1){
			...
			setDirections(params.primeColorLong.instructionsPre);
			$(".directions").show();
			blockNum ++;
		...
		}else if(taskTrialNum <= params.primeColorLong.trialsPerBlock * blockNum){
			...
			setTimeout(function(){rotateWheel(wheelRotation[taskTrialNum-1][1],"color",params.primeColorLong.wheelSize);},0);
			...
			setTimeout(function(){$(".colorCenter").hide(); $(".faceCenter").hide();}, 0);
			setTimeout(function(){setDirections(params.primeColorLong.instructionsBreak);}, 0);
		...
		setTimeout(function(){setDirections(params.primeColorLong.instructionsPost);}, 0);
		setTimeout(function(){$(".directions").show();}, 0);
		blockNum = -1;
		taskTrialNum = 1;
		//Next task
