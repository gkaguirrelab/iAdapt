(function() {
	...
	var faceArray = loadCSVfile(params.primeFaceLong.stimulusList);
	// Load the trial sequence from external file
	var temp = loadCSVfile(params.primeFaceLong.stimulusSeq);
	...
	var temp = loadCSVfile(params.primeFaceLong.primeSeq);
	var seqLen = temp.length;
	var primeSeq = [];
	...
	var temp = loadCSVfile(params.primeFaceLong.wheelRotation);
	...
	var preExperInstruction = loadTXTfile(params.primeFaceLong.instructionsPre);
	var breakInstruction = loadTXTfile(params.primeFaceLong.instructionsBreak);
	var postExperInstruction = loadTXTfile(params.primeFaceLong.instructionsPost);

	var trialsPerBlock = params.primeFaceLong.trialsPerBlock;
	var totalTrials = params.primeFaceLong.totalTrials;
	var screenHeight = screen.height;
	var windowHeight = $(window).height();
	var windowWidth = $(window).width();
	var divHeight = $("#content").height();
	var circleSize = divHeight * params.primeFaceLong.wheelSize;
	var centerSize = circleSize * params.primeFaceLong.centerSize;
	var calibratorSize = circleSize * params.primeFaceLong.centerSize;
	var thumbSize = circleSize * params.primeFaceLong.thumbSize;

	var textColor = params.primeFaceLong.textColor;
	var instructWidth = params.primeFaceLong.instructWidth;
	var instructHeight = params.primeFaceLong.instructHeight;
	var fontSize = params.primeFaceLong.instructFontsize;
	var buttonWidth = params.primeFaceLong.instructButtonwidth;
	var buttonHeight = params.primeFaceLong.instructButtonHeight;
	var buttonFontSize = params.primeFaceLong.instructButtonfontSize;
	var instructShiftFromTop = params.primeFaceLong.instructShiftFromTop;

	var minimumClickTime = params.primeFaceLong.minimumClickTime;
	var maxClickTime = params.primeFaceLong.maxClickTime;
	var primeDetect = params.primeFaceLong.primeDetect;
	...
	var trial = 0;
	var dbTableName = params.primeFaceLong.dbTableName;
	...
	function initializeExperiment() {
		...
		$("#content").css({"background-color": "rgb" +params.primeFaceLong.backgroundcolor});
		$("#header").css({"background-color": "rgb" +params.primeFaceLong.backgroundcolor});
		// Reset header text
		$("#header").html("");
		$("#header").hide();
		...
		//Set mask source
		$(".mask").attr('src',params.primeFaceLong.maskAddress);
		$(".mask").hide();
		// target
	...
	function setStyle(){
		...
		if($(window).width() != windowWidth || $(window).height() != windowHeight){
			...
			circleSize = divHeight * params.primeFaceLong.wheelSize;
			centerSize = circleSize * params.primeFaceLong.centerSize;
			calibratorSize = circleSize * params.primeFaceLong.centerSize;
			thumbSize = circleSize * params.primeFaceLong.thumbSize;
		}
		// Define the style of the face circle
		$("#progress-container").css({
		    'width': ''+(windowWidth*params.primeFaceLong.progressWidth)+'px',
		    'position': 'fixed',
		    'left':''+((windowWidth/2) - (windowWidth*params.primeFaceLong.progressWidth)/2)+'px',
		    'top':params.primeFaceLong.progressPercentFromTop +'%',
		    'height': windowHeight*params.primeFaceLong.progressHeight+'px',
		    'background-color':'rgb'+params.primeFaceLong.progressBackgroundColor
		});
		$("#progress-bar").css({
		  	'background-color':'rgb'+params.primeFaceLong.progressForegroundColor,
		  	'width': '0%',
		  	'height': windowHeight*params.primeFaceLong.progressHeight+'px',
		});
...
$("#directions2").css({
	...
	function executeTrial(){
		...
		$(".blank").show();
		setTimeout(function(){$(".blank").hide();time_after_blank0 = $.now(); time_before_prime = $.now();$(".prime").show();}, params.primeFaceLong.blank_before_prime_duration);
		setTimeout(function(){$(".prime").hide();time_after_prime = $.now(); time_before_blank1 = $.now();$(".blank").show();}, params.primeFaceLong.blank_before_prime_duration + params.primeFaceLong.prime_duration);
		setTimeout(function(){$(".blank").hide();time_after_blank1 = $.now(); time_before_mask = $.now();$(".mask").show();}, params.primeFaceLong.blank_before_prime_duration + params.primeFaceLong.prime_duration + params.primeFaceLong.blank_before_mask);
		setTimeout(function(){$(".mask").hide(); time_after_mask  = $.now(); time_before_blank2 = $.now();$(".blank").show();}, params.primeFaceLong.blank_before_prime_duration + params.primeFaceLong.prime_duration + params.primeFaceLong.blank_before_mask + params.primeFaceLong.mask_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank2  = $.now(); time_before_target  = $.now();$(".target").show();}, params.primeFaceLong.blank_before_prime_duration + params.primeFaceLong.prime_duration+ params.primeFaceLong.blank_before_mask + params.primeFaceLong.mask_duration + params.primeFaceLong.ISI);
		setTimeout(function(){$(".target").hide(); time_after_target = $.now(); time_before_blank3 = $.now(); $(".blank").show();}, params.primeFaceLong.blank_before_prime_duration + params.primeFaceLong.prime_duration+ params.primeFaceLong.blank_before_mask + params.primeFaceLong.mask_duration + params.primeFaceLong.ISI + params.primeFaceLong.target_duration);
		setTimeout(function(){$(".blank").hide(); time_after_blank3 = $.now();$(".face-circle").show();initialTime = $.now();if(maxClickTime != 0){$("#progress-container").show();loadProgressBar('progress-bar', maxClickTime, params.primeFaceLong.progressInterval);setMaxTimer(trial);}

},params.primeFaceLong.blank_before_prime_duration + params.primeFaceLong.prime_duration + params.primeFaceLong.blank_before_mask + params.primeFaceLong.mask_duration + params.primeFaceLong.ISI + params.primeFaceLong.target_duration + params.primeFaceLong.blank_after_target_duration);
	}
	function finalizeTrial(responseDegrees) {
		...
		writeTrialData(expData,trial,dbTableName, params.primeFaceLong.database);
		trial++;
	...
	function takeBreak(){
		...
		if(trial == 0){
			var moneyMadeSoFar = "";
			if(params.primeFaceLong.payByPerformance == 1){
				moneyMadeSoFar = "You have made " +totalEarned/100+"$ so far!\n"
			}
			if(params.primeFaceLong.displayExperNum == 1){setMessage(moneyMadeSoFar + "This is task " + currentExperNum + "/" + protocolList.length + "\n\n"+ preExperInstruction);}else{setMessage(preExperInstruction);}
			...
			var moneyMadeUpdate = "";
			if(params.primeFaceLong.payByPerformance == 1){
				totalEarned += Math.round(parseFloat(Math.max(totalScore, 0) *params.primeFaceLong.paymentFull));
				moneyMadeUpdate = "\nYou earned " +Math.round(parseFloat(Math.max(totalScore, 0) *params.primeFaceLong.paymentFull))+" cents for this task.";
				if(totalScore < params.primeFaceLong.minPerformance){
					poorAccuracy = 1;
				}
			}
	...
	$("#messageButton").click(function() {
		$(".messagedisplay").hide();
		if(trial == 0 && params.primeFaceLong.instructLoop != undefined){
			instructLoop(params.primeFaceLong.instructLoop.questions,
			params.primeFaceLong.instructLoop.choices,
			params.primeFaceLong.instructLoop.answers,initializeTrial);
			loopstatus = 1;
		}else{
			initializeTrial();
		}
...
function primeFaceLong(){
	...
	$("html").click(function(){
		if($(".color-circle").is(":visible")){
			...
			/*
				...
				if(task == "primeFaceLong" || task == "primeFaceLong"){
					//Account for wheel rotation
					if(task == "primeFaceLong"){
						var target = params["primeTargetColorLong"][taskTrialNum - 2] + rotation;
						if (target > 360) target -= 360;
						if (target < 0) target += 360;
						currPrime = calcPrimeOffset(target, params["primeprimeFaceLong"][taskTrialNum - 2]);
					}
					else{
						var target = params["primeprimeFaceLong"][taskTrialNum - 2] + rotation;
						if (target > 360) target -= 360;
						if (target < 0) target += 360;
						currPrime = calcPrimeOffset(target, params["primeprimeFaceLong"][taskTrialNum - 2]);
					}
	...
	if(taskTrialNum <= thisSeq.length){
		if(blockNum == -1){
			...
			setDirections(params.primeFaceLong.instructionsPre);
			$(".directions").show();
			blockNum ++;
		...
		}else if(taskTrialNum <= params.primeFaceLong.trialsPerBlock * blockNum){
			...
			setTimeout(function(){rotateWheel(wheelRotation[taskTrialNum-1][1],"color",params.primeFaceLong.wheelSize);},0);
			...
			setTimeout(function(){$(".colorCenter").hide(); $(".faceCenter").hide();}, 0);
			setTimeout(function(){setDirections(params.primeFaceLong.instructionsBreak);}, 0);
		...
		setTimeout(function(){setDirections(params.primeFaceLong.instructionsPost);}, 0);
		setTimeout(function(){$(".directions").show();}, 0);
		blockNum = -1;
		taskTrialNum = 1;
		//Next task
