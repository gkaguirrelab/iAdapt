...
for i=1:length(adaptorDur)
    for j=1:length(targetDur)
        for k=1:length(ISIDur)
            thisDBnameColor = ['db.data.prime_A' num2str(adaptorDur(i)) '_T' num2str(targetDur(j)) '_ISI' num2str(ISIDur(k)) 'Color_Mask'];
            thisDBnameFace = ['db.data.prime_A' num2str(adaptorDur(i)) '_T' num2str(targetDur(j)) '_ISI' num2str(ISIDur(k)) 'Face_Mask'];
...
combinations = genA_T_ISI_combin(targetDur, ISIDur);
xticks = [strcat(num2str(combinations(:, 1)), '/')  num2str(combinations(:,2))];
xticks = cellstr(xticks);
xticks = strrep(xticks, ' ', '');
...
set(gca, 'XTickLabel',{'50/100', '50/200', '50/400', '50/3000'}, 'YTick',1:numel({'50/100', '50/200', '50/400', '50/3000'}))
ylabel('Adaptor');
xlabel('Target/ISI');
...
combinations = genA_T_ISI_combin(targetDur, ISIDur, adaptorDur);
theLables = [strcat(num2str(combinations(:, 1)), '/')  strcat(num2str(combinations(:, 2)), '/') num2str(combinations(:,3))];
theLables = cellstr(theLables);
theLables = strrep(theLables, ' ', '');

figure(2)
labels = {'50/50/100', '50/50/200', '50/50/400', '50/50/3000', '200/50/100', '200/50/200', '200/50/400', '200/50/3000', '5000/50/100', '5000/50/200', '5000/50/400', '5000/50/3000'};
subplot(2,1,1)
